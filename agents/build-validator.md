...
|---|---|---|
| TS error in `src/data/portfolio.ts` | Wrong/missing field on a project object | Schema is `{ id, title, imgUrl, imgContain?, stack[], link }` |
| TS error in `src/data/timeline.ts` | Wrong/missing field on an entry | Schema is `{ id, year, type, title, duration, details[], chips[] }`; `type` must be `'work' \| 'education'` |
...
| `t('...')` returns a raw key | Key missing from `src/assets/lang/en.json` | Add the key to `en.json`; the `t()` helper falls back to the key string |
...
| Route not rendering | Not registered in `src/App.tsx` | Add a nested `<Route>` under `<Route path="/">` |
