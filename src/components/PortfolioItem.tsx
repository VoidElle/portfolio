...
const PortfolioItem: React.FC<Props> = ({ title, description, imgUrl, imgContain = false, stack, link, source, caseStudy, animDelay = 0 }) => {
    ...
    return (
        <div
            ref={ref}
            className={`group flex flex-col rounded-[14px] border border-subtle bg-surface overflow-hidden transition-[transform,box-shadow,border-color] duration-200 ease-out hover-hover:border-strong hover-hover:-translate-y-1 hover-hover:shadow-2xl ${inView ? 'animate-fade-up' : 'opacity-0'}`}
        ...
        >
            <div className="overflow-hidden">
                {imgUrl ? (
                    ...
                    <div
                        aria-hidden="true"
                        className="w-full h-48 flex items-center justify-center bg-accent-soft transition-transform duration-200 ease-out group-hover-hover:scale-105"
                    >
                    ...
                    <div className="flex flex-wrap gap-1.5 justify-end">
                        {caseStudy && (
                            <Link
                                ...
                                className="inline-flex items-center gap-1 rounded-full border border-subtle text-[0.68rem] font-semibold text-muted px-2.5 py-0.5 transition-[color,border-color,transform] active:scale-[0.97] hover:text-fg hover:border-strong group-hover:text-fg group-hover:border-strong"
                            >
